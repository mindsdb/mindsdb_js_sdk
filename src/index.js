...
const installDependencies = (name) => {
	...
	return connection.api.get(request).then((res) => {
		if (res.status === 500) {
			return res.json();
		}
	});
...
class Predictor {
	...
	rename = async (params) => {
		...
		const request = setQueryParams(mergeParams, `/predictors/${params.oldName}/rename?new_name=${params.newName}`);
		const response = await connection.api.get(request);
	...
	update = async (params, predictorName) => {
		...
		const request = setQueryParams(mergeParams, `/predictors/update/${predictorName}`);
		const response = await connection.api.get(request);
...
class DataSource {
	...
	getDownloadUrl = () =>
		this.source_type === 'url' ? this.source : `${connection.url}/datasources/${this.name}/download`;
